...
const invoiceData = [
  { id: 'INV-1041', client: 'Acme Corp', amount: '$12,400', status: 'paid' as const, due: 'Jan 15' },
  { id: 'INV-1042', client: 'TechStart Ltd', amount: '$8,750', status: 'pending' as const, due: 'Feb 01' },
  { id: 'INV-1043', client: 'GlobalFin Inc', amount: '$24,000', status: 'overdue' as const, due: 'Jan 28' },
  { id: 'INV-1044', client: 'RetailMax', amount: '$6,200', status: 'paid' as const, due: 'Jan 20' },
  { id: 'INV-1045', client: 'DataFlow Systems', amount: '$15,800', status: 'draft' as const, due: 'Feb 10' },
  { id: 'INV-1046', client: 'Bright Solutions', amount: '$3,900', status: 'pending' as const, due: 'Feb 05' },
];
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-[1fr_1.2fr_0.8fr_0.7fr_0.7fr] gap-2 px-2 pb-2 border-b border-gray-200">
        <span className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Invoice</span>
        <span className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Client</span>
        ...
        <span className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide">Due</span>
      </div>
      ...
      <div className="flex items-center justify-between pt-2 mt-1 border-t border-gray-200">
        <span className="text-[10px] text-gray-500">Outstanding: <span className="font-semibold text-gray-900">{outstanding}</span></span>
        <span className="text-[10px] text-red-600 font-medium">{overdueCount} overdue</span>
      </div>
    </div>
  );
};
...
const expenseData = [
  { id: 'e1', desc: 'Flight to Munich', date: 'Jan 22', amount: '$840', category: 'Travel', status: 'approved' as const },
  { id: 'e2', desc: 'Figma annual license', date: 'Jan 18', amount: '$180', category: 'Software', status: 'approved' as const },
  { id: 'e3', desc: 'Team lunch — Q4 review', date: 'Jan 25', amount: '$320', category: 'Office', status: 'pending' as const },
  { id: 'e4', desc: 'Legal consultation', date: 'Jan 28', amount: '$1,200', category: 'Services', status: 'pending' as const },
  { id: 'e5', desc: 'Desk monitors (x3)', date: 'Jan 30', amount: '$2,100', category: 'Office', status: 'pending' as const },
];
      ...
      <div className="flex-1 space-y-1.5 overflow-y-auto">
        {filtered.map((exp) => (
          ...
          >
            ...
            <span className="text-[11px] font-semibold text-gray-900 w-16 text-right">{exp.amount}</span>
            <span className={cn(
...
const approvalItems = [
  { id: 'a1', type: 'expense', desc: 'Team lunch — Q4 review', requester: 'SK', amount: '$320', submitted: '2h ago', status: 'pending' as const },
  { id: 'a2', type: 'invoice', desc: 'Vendor payment — DesignCo', requester: 'JD', amount: '$4,500', submitted: '5h ago', status: 'pending' as const },
  { id: 'a3', type: 'expense', desc: 'Desk monitors (x3)', requester: 'MR', amount: '$2,100', submitted: '1d ago', status: 'pending' as const },
  { id: 'a4', type: 'expense', desc: 'Client dinner', requester: 'JD', amount: '$185', submitted: '2d ago', status: 'approved' as const },
];
...
export const ApprovalsQueueRealistic: React.FC<{ className?: string }> = ({ className }) => {
  ...
  return (
    <div
      ...
      onMouseLeave={() => setIsHovered(false)}
    >
      <div className="text-xs font-medium text-gray-700 mb-3">Pending Approvals</div>
      <div className="flex-1 space-y-1.5 overflow-y-auto">
        {items.map((item) => (
          <div
            key={item.id}
...
const financeKPIs = [
  { label: 'Revenue', value: '$84.2k', trend: 'up' as const, change: '+8%', icon: DollarSign },
  { label: 'Expenses', value: '$52.1k', trend: 'up' as const, change: '+3%', icon: CreditCard },
  { label: 'Net Income', value: '$32.1k', trend: 'up' as const, change: '+15%', icon: TrendingUp },
  { label: 'Outstanding', value: '$30.5k', trend: 'down' as const, change: '-12%', icon: Receipt },
];
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 bg-white rounded-lg border border-gray-200 p-3">
        <div className="text-[10px] font-medium text-gray-500 mb-2">Revenue vs Expenses</div>
