...
export const ProductPreviewFrame: React.FC<ProductPreviewFrameProps> = ({
  ...
  return (
    <div className={cn(
      ...
      <div className={cn('bg-gray-100 border-b border-gray-200 px-4 py-3 flex items-center gap-3', isRTL && 'flex-row-reverse')}>
        ...
        <div className="flex-1 max-w-md">
          <div className="bg-white rounded-md px-3 py-1.5 text-xs text-gray-500 border border-gray-200">
            ...
            <span className={cn(isRTL ? 'mr-2' : 'ml-2')}>
              {(locale === 'ar' ? 'moayena' : title.toLowerCase().replace(/\s+/g, '')).replace(/\s+/g, '')}.adsi.cloud
            </span>
          </div>
        </div>
      </div>
